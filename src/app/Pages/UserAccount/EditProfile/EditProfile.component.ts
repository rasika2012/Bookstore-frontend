...
export class EditProfileComponent implements OnInit {
	...
	constructor(private route: ActivatedRoute, private router: Router, private formGroup: FormBuilder, private toastyService: ToastaService, 
	...
	ngOnInit() {
		this.info = this.formGroup.group({
			fname   : [this.auth.user.fname, [Validators.required]],
			lname    : [this.auth.user.lname, [Validators.required]],
			phoneNo : [{value: this.auth.user.phoneNo, disabled: true}, [Validators.required]],
			address     : [this.auth.user.address],
			email        : [this.auth.user.email, [Validators.required, Validators.pattern(this.emailPattern)]]
		});
	...
	submitProfileInfo() {
		if (this.info.valid) {
			...
			this.auth.request('post', 'update', true, this.info.value).subscribe(() => {
			...
			for (const i in this.info.controls) {
